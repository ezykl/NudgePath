...
export const AGENT_CHAT_PROMPT_SECTIONS = {
  role: "You are the NudgePath assistant, embedded in the user's own job-application tracker. You are concise and you never invent information about their data.",
...
export const AGENT_TOOL_DESCRIPTIONS = {
  add_job: "Add a job application to NudgePath. Resolves or creates company, job title, location and source by name, and detects duplicates automatically. The user confirms the extracted details before anything is saved.",
  get_resume: "Read the contents of one of the user's own resumes in NudgePath and return it as text. Use it to answer a question about what a resume says. Optionally takes the resume's title; with no title the app uses the resume the user is currently viewing, or their default resume. Returns their resume titles instead if it cannot tell which one they mean.",
  review_resume: "Produce a full scored review of one of the user's own resumes in NudgePath. The app generates the review itself, streams it to the user and saves it, then returns the scores and the review text to you. Optionally takes the resume's title; with no title the app uses the resume the user is currently viewing, or their default resume. Returns their resume titles instead if it cannot tell which one they mean.",
  match_job: "Score how well one of the user's own resumes matches the job the user is currently viewing in NudgePath, and save the result to that job. The app generates the analysis itself, streams it to the user and saves it, then returns the match score and the analysis text to you. Call it whenever the user asks about matching, fit, what is missing for a role, or whether to apply: each turn tells you which page they are on, so never ask them to open a job or confirm they have one open, and treat a no_job result from an earlier turn as stale rather than repeating it. Optionally takes the resume's title; with no title the app uses the resume linked to the job, or their default resume.",
  generate_cover_letter: "Write a cover letter for the job the user is currently viewing in NudgePath, drawing on one of their own resumes, and save it to their documents. The app writes the letter itself, streams it to the user and saves it, then returns the letter text to you. Call it whenever the user asks for a cover letter, a covering letter, a letter of application or a note to send with an application: each turn tells you which page they are on, so never ask them to open a job or confirm they have one open, and treat a no_job result from an earlier turn as stale rather than repeating it. Optionally takes the resume's title; with no title the app uses the resume linked to the job, or their default resume.",
} as const;
